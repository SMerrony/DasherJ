...
/**
 * Crt represents the glass screen of the visual display Terminal. Almost no
 * emulation logic here - that all happens in Terminal
 *
 * All painting to the main screen area happens here.
 *
 * N.B. This class does not need to be aware of the physical screen (or window)
 * geometry, it simply draws pixels/images on the Canvas. All scaling and sizing
 ...
 * v. 0.5 - Remove 'rasterised' font look Improve performance and realism by eliminating
 * affinetransform Fix printing to scale and position reasonably
 *
 */
public class Crt extends Canvas {
    ...
    public Crt(Terminal terminal) {
        ...
        if (!bdfFont.load(getClass().getResourceAsStream(DASHER_FONT_BDF))) {
            System.out.printf("Crt: Fatal Error - Could not load custom Dasher font.%n");
            System.exit(1);
        }
    ...
    /**
     * *
     * Paint individual character cells onto the passed in Graphics object
     * (usually the Crt - but may be a separate image for printing etc...)
     *
